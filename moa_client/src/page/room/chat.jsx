...
class Chat extends Component {
  constructor(props) {
    super(props);
    this.state = { name: "", filename: "", file_originalname: "" };
  }

  /////
  // 채팅 및 파일 경로 DB에 저장
  /////
  chatSave = async (filename, file_originalname) => {
    const send_param = {
      room_id: this.props.room,
      name: this.state.name,
      message: this.test.value,
      filename,
      file_originalname,
    };
    try {
  ...
  componentDidMount = () => {
    /* 서버에 새로운 유저가 왔다고 알림 */
    this.props.io.emit("newUser");
    this.props.io.on("name", (data) => {
  ...
  fileUpload = async (event) => {
    ...
    try {
      ...
      if (result.data.resultCode) {
        ...
        this.fileUpload2();
        this.chatSave(result.data.filename, result.data.file_originalname);
      } else {
